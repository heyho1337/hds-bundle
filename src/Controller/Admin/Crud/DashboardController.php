...
class DashboardController extends AbstractDashboardController
{
    ...
    public function configureMenuItems(): iterable
    {
        ...
        foreach($this->component as $group => $menuItems){
            yield MenuItem::section($this->translateService->translateSzavak($group));
            if($group === 'basic'){
                ...
                if($this->hasAccess($menu->getRole())){
                    if($menu->getName() === 'config'){
                        yield MenuItem::linkToCrud(
                            $this->translateService->translateSzavak($menu->getName(),$menu->getLabel()), 
                            $menu->getIcon(), 
                            $entityClass
                        )
                        ->setAction('edit')
            ...
            yield MenuItem::section($this->translateService->translateSzavak("languages"));
                ...
                foreach ($this->langs as $locale) {
                    ...
                    yield MenuItem::linkToUrl(
                        $this->translateService->translateSzavak($locale->getCode()."_desc",$locale->getName()),
                        'fa fa-globe',
                        $url
                    );
                }
